...
do
    ...
    wtl-log docker-images-export.sh 7 DOCKER_IMAGES_EXPORT_CURRENT "Image: "$img
    DIRNAME=$WTL_CACHE"/docker-images/"$img_name"/"
    test -d $DIRNAME || mkdir -p $DIRNAME
    FILENAME=$DIRNAME$img_tag".tar"
    if test ! -f $FILENAME ; then
        wtl-log docker-images-export.sh 7 DOCKER_IMAGES_EXPORT_CURRENT_FILE "Export to "$FILENAME
