...
if [[ "$WTL_INSTANCE_NAME" == "" ]] ; then
    wtl-log scripts/backup-auto-delete.sh 4 MIGGING_WTL_INSTANCE_NAME "You need the WTL_INSTANCE_NAME env"
    exit 1
fi
...
while [[ $(ls $WTL_BACKUPS | grep -v StaticBackup | wc -l) -gt $WTL_BACKUPS_MAX_NUM ]] ; do
    ...
    wtl-log scripts/backup-auto-delete.sh 4 BACKUP_AUTO_DELETE_DO_FOR "Deleting the backup: "$BACKUP_TO_DELETE
