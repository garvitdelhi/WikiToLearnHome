...
set -e
if [[ $(basename $0) != "mw-dumps-do-production.sh" ]] ; then
    echo "Wrong way to execute mw-dumps-do-production.sh"
    exit 1
fi
...
if docker inspect wikitolearn-haproxy &> /dev/null ; then
    ...
    wtl-log mw-dumps-do-production.sh 0 MISSING_HA_PROXY "Production backup-do: missing docker wikitolearn-haproxy"
fi
