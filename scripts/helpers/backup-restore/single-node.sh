...
for dbname in $(cat $WTL_WORKING_DIR/databases.conf) ; do
    ...
    if [[ $TABLE_COUNT -ne 0 ]] ; then
        wtl-log scripts/helpers/backup-restore/single-node.sh 4 RESTORE_NON_EMPTY_DB "The "$dbname" is not empty ("$TABLE_COUNT" tables)"
        exit 1
    fi
done
...
for dbname in $(cat $WTL_WORKING_DIR/databases.conf) ; do
    ...
    then
        wtl-log scripts/helpers/backup-restore/single-node.sh 4 RESTORE_DB "Restore for $dbname"
        ...
        then
            wtl-log scripts/helpers/backup-restore/single-node.sh 4 RESTORE_DB_DEVELOPER_DUMP "Restore for $dbname from DeveloperDump"
            cat $WTL_REPO_DIR"/DeveloperDump/"$dbname".struct.sql" | docker exec -i ${WTL_INSTANCE_NAME}-mysql mysql $dbname
        else
            wtl-log scripts/helpers/backup-restore/single-node.sh 4 RESTORE_DB_MISSING_DUMP "Missing dump for $dbname"
        fi
    fi
done
