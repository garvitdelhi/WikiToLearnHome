...
wtl-log scripts/helpers/backup-do/single-node.sh 3 BACKUP_STARTED "Started backup for "${WTL_INSTANCE_NAME}

rsync -a --stats --delete $WTL_WORKING_DIR"/mediawiki/images/" ${BACKUP_DIR}"/images/"

wtl-log scripts/helpers/backup-do/single-node.sh 3 BACKUP_RO "Instance "${WTL_INSTANCE_NAME}" read only"
...
for db in $(docker exec -ti ${WTL_INSTANCE_NAME}-mysql mysql -e "SHOW DATABASES" | grep wikitolearn | awk '{ print $2 }') ; do
 wtl-log scripts/helpers/backup-do/single-node.sh 3 BACKUP_DB "Backup "$db
...
wtl-log scripts/helpers/backup-do/single-node.sh 3 BACKUP_FINISHED "Backup for "${WTL_INSTANCE_NAME}" finished"
