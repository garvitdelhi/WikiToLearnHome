...
wtl-log scripts/helpers/create/single-node-no-ocg.sh 3 NN "[create/single-node-no-ocg] Running in PRODUCTION mode!"

$WTL_SCRIPTS"/helpers/create/commons/single-node-pre.sh"

if ! docker inspect ${WTL_INSTANCE_NAME}-websrv &> /dev/null ; then

    wtl-log scripts/helpers/create/single-node-no-ocg.sh 3 NN "[create/single-node-no-ocg] Creating docker ${WTL_INSTANCE_NAME}-websrv"
    ...
    wtl-log scripts/helpers/create/single-node-no-ocg.sh 3 NN "[create/single-node-no-ocg] Copying certs to websrv"
    if ! docker inspect ${WTL_INSTANCE_NAME}-websrv &> /dev/null ; then
        TMPDIR=`mktemp -d`
        chmod 700 $TMPDIR
        mkdir $TMPDIR/certs
        ...
        if ! docker cp ${TMPDIR}/certs/ ${WTL_INSTANCE_NAME}-websrv:/certs/ ; then
            rm ${TMPDIR} -Rf
            wtl-log scripts/helpers/create/single-node-no-ocg.sh 3 NN "[create/single-node-no-ocg] Error: Unable to copy wikitolearn.crt to the webserver"
            exit 1
        fi
        rm ${TMPDIR} -Rf
    fi
