...
if ! docker inspect ${WTL_INSTANCE_NAME}-websrv &> /dev/null ; then
    ...
    wtl-log scripts/helpers/create/single-node-ocg-develop.sh 3 CREATE_DOCKERS "Creating docker ${WTL_INSTANCE_NAME}-websrv"
    ...
    wtl-log scripts/helpers/create/single-node-ocg-develop.sh 3 CREATE_COPY_WEBSRV_CERTS "Copying certs to websrv"
    if ! docker inspect ${WTL_INSTANCE_NAME}-websrv &> /dev/null ; then
        TMPDIR=`mktemp -d`
        chmod 700 $TMPDIR
        mkdir $TMPDIR/certs
        ...
        if ! docker cp ${TMPDIR}/certs/ ${WTL_INSTANCE_NAME}-websrv:/certs/ ; then
            rm ${TMPDIR} -Rf
            wtl-log scripts/helpers/create/single-node-ocg-develop.sh 0 CREATE_COPY_WEBSRV_CERTS_FAIL "Error: Unable to copy wikitolearn.crt to the webserver"
            exit 1
        fi
        rm ${TMPDIR} -Rf
    fi
