...
if ! docker start ${WTL_INSTANCE_NAME}-websrv ; then
    wtl-log scripts/helpers/start/commons/single-node-websrv.sh 3 NN "[start/single-node] FATAL ERROR: MISSING WEBSRV"
    exit 1
fi

if [[ ! -f $WTL_CONFIGS_DIR/LocalSettings.d/wgSecretKey.php ]] ; then
    wtl-log scripts/helpers/start/commons/single-node-websrv.sh 3 NN "[start/single-node] Creating new '$WTL_CONFIGS_DIR/LocalSettings.d/wgSecretKey.php' file"
    ...
    wtl-log scripts/helpers/start/commons/single-node-websrv.sh 3 NN "[start/single-node] Using the existent '$WTL_CONFIGS_DIR/LocalSettings.d/wgSecretKey.php' file"
fi

if [[ "$WTL_MAIL_RELAY_HOST" != "" ]] ; then
    wtl-log scripts/helpers/start/commons/single-node-websrv.sh 3 NN "[start/single-node] Setup the relay host to "${WTL_MAIL_RELAY_HOST}
    {
