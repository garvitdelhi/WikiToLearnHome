...
if [[ -f $WTL_WORKING_DIR"/mediawiki.version" ]] ; then
    ...
    if [[ "$MW_MAJOR" == "snapshot" ]] ; then
        ...
        mkdir mediawiki
        cd mediawiki
        wtl-log scripts/download-mediawiki-extensions.sh 7 DOWNLOAD_MEDIAWIKI_EXTENSIONS_CORE_SNAPSHOT "Mediawiki snapshot:"$MW_MINOR
        ...
        wtl-log scripts/download-mediawiki-extensions.sh 7 DOWNLOAD_MEDIAWIKI_EXTENSIONS_CORE "Mediawiki: "$MW_MAJOR"."$MW_MINOR
        tar xfz $WTL_CACHE"/download/"$_MW_FILE
...
if [[ -f $WTL_WORKING_DIR"/extensions.list.version" ]] ; then
    ...
    do
        ...
        wtl-log scripts/download-mediawiki-extensions.sh 7 DOWNLOAD_MEDIAWIKI_EXTENSIONS_EXT "Extension: "$extension" ("$extension_version")"
        if test -d $extension ; then
            rm -Rf $extension
        fi
