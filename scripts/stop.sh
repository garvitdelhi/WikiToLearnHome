...
if [[ $(docker ps -aq  --filter "name=$WTL_INSTANCE_NAME" | wc -l) -gt 0 ]] ; then
    wtl-log scripts/stop.sh 3 STOP_DOING "Stopping Dockers with prefix '$WTL_INSTANCE_NAME'"
    docker stop $(docker ps -aq  --filter "name=$WTL_INSTANCE_NAME")
    wtl-log scripts/stop.sh 3 STOP_DONE "Stopped Dockers with prefix '$WTL_INSTANCE_NAME'"
fi
