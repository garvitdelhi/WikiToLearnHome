...
if [[ ! -d $WTL_CACHE/composer ]] ; then
    echo "do-one-composer: creating new cache dir"
    mkdir -p $WTL_CACHE/composer
fi

docker run --rm -u $WTL_USER_UID:$WTL_USER_GID -v "$DO_ONE_COMPOSER_DIR":/app -e COMPOSER_CACHE_DIR=/cache -e COMPOSER_HOME=/composer -v $WTL_CACHE/composer:/cache -v $WTL_CONFIGS_DIR/composer:/composer composer/composer install
